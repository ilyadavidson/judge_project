...
from jp.utils.io    import _load_mapping, load_case_results

def get_id_from_names(out: pd.DataFrame, judges: pd.DataFrame) -> pd.Series:
    """
    This function matches judge IDs with judge names from the API answers. 

    :param out: DataFrame containing API answers with judge names (lower_judge_first, lower_judge_last).
    :param judges: DataFrame containing judge information with columns 'first name', 'last name', and 'judge id'.
    """
...
def attach_api_to_cl_clean(
    ...
    api_path:           str = "data/artifacts/api/outputs/cases_answers.jsonl",
    ...
    *,
    ...
    """
    Unlike the CAP dataset, the CL data has the district judge in the opinion text so we don't need to do mapping. 
    This function attaches API answers to the cleaned CL data and matches district judge IDs and names that are already given.
    """
    ...
    # Ensure unique_id is string for merging
    out["custom_id"] = (
    out["unique_id"]
    .astype("string")
)
